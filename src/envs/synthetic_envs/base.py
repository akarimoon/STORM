...
from envs.synthetic_envs.utils import norm
...
class BaseEnv:
    ...
    def _draw_objs(self, objs, mode="rgb_array"):
        ...
        if mode == "rgb_array" or self._num_stacked_obss == 1:
            return obs.reshape(1, *obs.shape).copy() 
        else:
    ...
    def reset(self):
        ...
        return self.render(), {"episode_frame_number": self.step_count}
    ...
    def step(self, act):
        ...
        if self._rew_type == "dense":
            if self._get_dist(self._target_obj_idx, -1) < dist_before_acting:
                ...
                reward = -0.01
        return reward, False, done, {"episode_frame_number": self.step_count}
