...
class OCQuantizedReconstructor(nn.Module):
    ...
    def update(self, obs, action, reward, termination, logger=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=self.use_amp):            
            ...
            recon_logits, _, mask_as_image = self.dist_head.forward_prior(slots)
            ...
            if self.loss_config.recon_x_from_slots:
                ...
                total_loss += coef_ * recon_curr_loss
...
class OCQuantizedVideoPredictor(nn.Module):
    ...
    def load(self, path_to_checkpoint, device):
        ...
        self.out2.load_state_dict(extract_state_dict(state_dict, 'out2'))
    ...
    def predict_next(self, last_sample, last_hidden, action, log_video=True):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=self.use_amp):
            ...
            prior_logits, _, mask_as_image = self.dist_head2.forward_prior(hidden_hat_proj)
            if self.sbd_target == 'onehot':
                prior_logits = rearrange(prior_logits, "B L K C -> (B L) K C")
                prior_logits = self.out2(prior_logits)
                sample = self.dist_head2.quantizer.indices_to_codes(prior_logits.argmax(dim=-1))
                sample = rearrange(sample, "(B L) K C -> B L K C", B=batch_size)
            elif self.sbd_target == 'soft':
                sample = prior_logits
    ...
    def update(self, obs, action, reward, termination, logger=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=self.use_amp):            
            ...
            # slots_hat = self.storm_transformer(slots.detach(), action, temporal_mask) # B L N D
            ...
            slots_hat = self.slot_proj(slots_hat)
            slots_hat, prior_mu, prior_sigma = self.sample(slots_hat, dist="prior")
            prior_logits, _, mask_as_image = self.dist_head2.forward_prior(slots_hat)
            prior_logits = rearrange(prior_logits, "B L K C -> (B L) K C")
            if self.sbd_target == 'onehot':
                prior_logits = self.out2(prior_logits)
                prior_sample = self.dist_head2.quantizer.indices_to_codes(prior_logits.argmax(dim=-1))
            elif self.sbd_target == 'soft':
                prior_sample = prior_logits
