...
class DistHead(nn.Module):
    ...
    def forward_post(self, x, tau):
        ...
        out, indices = self.quantizer(x)
        soft = rearrange(out, "B L (H W) C -> B L C H W", H=H, W=W)
        hard = F.one_hot(indices, self.quantizer.codebook_size).float()
        return soft, hard
...
class SLATEwithFSQ(nn.Module):
    def __init__(self, in_channels, action_dim, stem_channels, stoch_num_classes, stoch_dim, num_slots, slot_dim, dec_hidden_dim, dec_num_layers, vocab_size, post_type,
        ...
        self.pos_embed = nn.Sequential(
            PositionalEncoding1D(self.dist_head.quantizer.codebook_size, stoch_dim, weight_init="trunc_normal"),
            nn.Dropout(0.1)
        )
        self.out = nn.Linear(stoch_dim, self.dist_head.quantizer.codebook_size, bias=False)
    ...
    def inspect(self, obs, tau=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=self.use_amp):
            ...
            z_emb = self.dist_head.quantizer.indices_to_codes(tokens)
            ...
            pred_logits = self.dist_head.quantizer.indices_to_codes(pred_logits.argmax(dim=-1))
...
class SLATEWorldModel(SLATEwithFSQ):
    def __init__(self, in_channels, action_dim, stem_channels, stoch_num_classes, stoch_dim, num_slots, slot_dim, dec_hidden_dim, dec_num_layers, vocab_size, post_type,
